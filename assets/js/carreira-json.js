/*
    Versão orientada por JSON.

    Este arquivo mostra uma separação comum em projetos reais:
    - dados editáveis ficam no JSON;
    - estrutura fica no HTML;
    - montagem dinâmica fica no JavaScript.
*/
...
const renderProfile = ({ profile, contacts }) => {
    ...
    cvLink.setAttribute("aria-label", `Baixar currículo de ${profile.name} em PDF`);
    ...
    contacts.forEach((contact) => {
        ...
        // CORREÇÃO AQUI: Mudado para aceitar tanto "http" quanto "https" corretamente
        if (contact.url.startsWith("http://") || contact.url.startsWith("https://")) {
            link.target = "_blank";
            link.rel = "noopener noreferrer"; // Boa prática de segurança para links externos
        }
...
fetch(DATA_URL)
    ...
    .catch((error) => {
        ...
        if (main) {
            const warning = createElement("p", "alert alert-warning m-5", "Não foi possível carregar os dados do JSON. Execute a página em um servidor local ou publique no GitHub Pages.");
